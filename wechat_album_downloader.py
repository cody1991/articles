...
class WeChatAlbumDownloader:
    ...
    def download_all(self, reverse=True, download_content=True):
        """
        下载所有文章
...
def main():
    # 合集URL
    album_url = "https://mp.weixin.qq.com/mp/appmsgalbum?__biz=Mzg2NTkwNTM4MA==&action=getalbum&album_id=3896715541905326087"
    
    # 创建下载器
    downloader = WeChatAlbumDownloader(album_url, output_dir="wechat_articles")
    
    # 下载所有文章（倒序）
    # 设置 download_content=False 可以只获取列表不下载内容
    downloader.download_all(reverse=True, download_content=True)
